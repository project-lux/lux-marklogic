...
const SearchCriteriaProcessorOriginal = class {
  ...
  process(
    ...
    this.resolvedSearchCriteria =
      SearchCriteriaProcessorOriginal._requireSearchCriteriaJson(
        scopeName,
        searchCriteria,
      );
    ...
    if (this.scopeName === 'multi') {
      ...
      if (this.resolvedSearchCriteria.OR) {
        ...
        SearchCriteriaProcessorOriginal._requireSearchCriteriaArray(orArr);
        if (orArr.length === 0) {
          ...
          this.ctsQueryStr = `cts.orQuery([${orArr.map(
            (subCriteria, index, array) => {
              ...
              const { filterResults, facetsAreLikely, synonymsEnabled } =
                this.requestOptions;
              const searchCriteriaProcessor =
                new SearchCriteriaProcessorOriginal(
                  filterResults,
                  facetsAreLikely,
                  synonymsEnabled,
                );
  ...
  getEstimate() {
    return cts.estimate(
      SearchCriteriaProcessorOriginal.evalQueryString(this.getCtsQueryStr()),
    );
  }
  ...
  getSearchResults() {
    const sortType =
      SearchCriteriaProcessorOriginal.getSortTypeFromSortCriteria(
        this.sortCriteria,
      );
  ...
  _getMultiScopeSortResults() {
    const ctsQuery = SearchCriteriaProcessorOriginal.evalQueryString(
      this.getCtsQueryStr(),
    );
  ...
  _getNonSemanticSortResults() {
    ...
    if (this.pageWith) {
      ...
      const docs = fn
        .subsequence(
          cts.search(
            SearchCriteriaProcessorOriginal.evalQueryString(
              this.getCtsQueryStr(),
            ),
            searchOptionsArr,
          ),
  ...
  generateQueryFromCriteria(
    ...
    let ctsQueryStr = '';
    SearchCriteriaProcessorOriginal._requireSearchCriteriaObject(
      searchCriteria,
    );
    ...
    if (searchCriteria.AND || searchCriteria.OR) {
      ...
      SearchCriteriaProcessorOriginal._requireSearchCriteriaArray(groupArr);
  ...
  _parseAndValidateTerm(
    ...
    if (utils.isString(termValue)) {
      const tokenizedValues =
        SearchCriteriaProcessorOriginal._tokenizeSearchTermValue(
    ...
    if (typeof termValue == 'object') {
      ...
      if (
        SearchCriteriaProcessorOriginal._hasIdChildTerm(termValue) &&
        ...
        if (SearchCriteriaProcessorOriginal._hasGroup(termValue)) {
          ...
          SearchCriteriaProcessorOriginal.hasNonOptionPropertyName(termValue)
        ) {
    ...
    SearchCriteriaProcessorOriginal._cleanTermValues(searchTerm);
  ...
  _getPartialChildSearchTermInfo(scopeName, termValue) {
    const hasGroup = SearchCriteriaProcessorOriginal._hasGroup(termValue);
    ...
    if (!hasGroup) {
      const termName =
        SearchCriteriaProcessorOriginal.getFirstNonOptionPropertyName(
          termValue,
        );
      const searchTermConfig = this._getSearchTermConfig(scopeName, termName);
  ...
  static _cleanTermValues(searchTerm) {
    if (SearchCriteriaProcessorOriginal._isKeywordTerm(searchTerm)) {
  ...
  static _requireSearchCriteriaJson(scopeName, searchCriteria) {
    ...
    return SearchCriteriaProcessorOriginal.translateStringGrammarToJSON(
      scopeName,
      searchCriteria,
    );
  }
  ...
  static translateStringGrammarToJSON(scopeName, searchCriteria) {
    ...
    const adjustedSearchCriteriaStr =
      SearchCriteriaProcessorOriginal._adjustSearchString(searchCriteria);
    ...
    return {
      _scope: scopeName,
      ...SearchCriteriaProcessorOriginal._walkParsedQuery(ctsQueryObj),
    };
  }
        ...
        if (utils.isNonEmptyArray(ctsQueryObj[propName].queries)) {
          for (const item of ctsQueryObj[propName].queries) {
            searchCriteriaJson[operator].push(
              SearchCriteriaProcessorOriginal._walkParsedQuery(item),
            );
          }
        }
      } else if (propName == 'notQuery') {
        searchCriteriaJson.NOT = [
          SearchCriteriaProcessorOriginal._walkParsedQuery(
  ...
  static getSortTypeFromSortBinding(sortBinding) {
    if (utils.isObject(sortBinding)) {
      ...
      return SearchCriteriaProcessorOriginal.getSortType(
        isMultiScope,
        isSemantic,
      );
    }
...
export { START_OF_GENERATED_QUERY, SearchCriteriaProcessorOriginal };
