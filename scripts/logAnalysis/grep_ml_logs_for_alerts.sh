...
usage()
{
  ...
  echo
  echo e.g. $0 green 8003_ErrorLog.txt
  echo
}
 ...
 *)
 ...
 do
  echo $s
# echo ssh -i ./ch-lux-ssh-${env}.pem ec2-user@${ip}  grep \"${s}\" /var/opt/MarkLogic/Logs/ErrorLog.txt
       ssh -i ./ch-lux-ssh-${env}.pem ec2-user@${ip}  grep \"${s}\" /var/opt/MarkLogic/Logs/${log}
# echo ""
 done
done
