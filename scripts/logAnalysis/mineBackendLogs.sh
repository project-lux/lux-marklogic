...
AT_LEAST_ONE_RELATED_LIST_ITEM_FILE=$OUTPUT_DIRECTORY/atLeastOneRelatedLitItem.txt
AT_LEAST_ONE_RELATED_LIST_ITEM_TSV_FILE=$OUTPUT_DIRECTORY/atLeastOneRelatedLitItem.tsv
...
cat "$SEARCH_PARAMS_WITH_DURATIONS_FILE" >> "$SEARCH_PARAMS_WITH_DURATIONS_TSV_FILE"
# sed only supports 9 backreferences, but we need 10.  awk supports more than 9 but was taking too long to figure out.
# Keep the following regular expression and that of the next one are to be identical.
sed -i 's/.*txt:\(.*\) Info: .* requestId: \([0-9]*\); requestContext: \([a-zA-Z]*\); totalElapsed: \([0-9]*\); searchElapsed: \([0-9]*\); estimate: \([^;]*\); returned: \([0-9]*\); scope: \[\([a-z ,]*\)\]; searchCriteria: \[\(.*\)\]/\1\t\2\t\3\t\4\t\5\t\6\t\7\t\8\t\9/' "$SEARCH_PARAMS_WITH_DURATIONS_TSV_FILE"
...
cat "$SEARCH_PARAMS_WITH_DURATIONS_FILE" >> "$SEARCH_PARAMS_WITH_DURATIONS_JSON_FILE"
# The following sed command expects its input lines to begin with a file name.
sed -i 's/.*txt:\(.*\) Info: .* requestId: \([0-9]*\); requestContext: \([a-zA-Z]*\); totalElapsed: \([0-9]*\); searchElapsed: \([0-9]*\); estimate: \([^;]*\); returned: \([0-9]*\); scope: \[\([a-z ,]*\)\]; searchCriteria: \[\(.*\)\]/{"timestamp": "\1", "requestId": "\2", "requestContext": "\3", "total": \4, "search": \5, "estimate": \6, "returned": \7, "scope": "\8", "criteria": \9},/' "$SEARCH_PARAMS_WITH_DURATIONS_JSON_FILE"
