...
const SearchCriteriaProcessor = class {
  ...
  getEstimate() {
    // Use Optic API for consistency with unified search approach
    const ctsQuery = SearchCriteriaProcessor.evalQueryString(
      this.getCtsQueryStr(),
    );
    return op.fromSearch(ctsQuery).count();
  }

  // returns { resultPage: number, results: Array<{id: string, type: string}> }
  getSearchResults() {
    return this._getUnifiedSearchResults();
  }

  // Unified search implementation using Optic API for all sort types
  _getUnifiedSearchResults() {
    const ctsQuery = SearchCriteriaProcessor.evalQueryString(
      this.getCtsQueryStr(),
    );
    ...
    // Optimization: Choose optimal plan construction strategy
    const planStrategy = this._selectOptimalPlanStrategy(sortType, ctsQuery);

    switch (planStrategy) {
      case 'lexicon-first':
        return this._getLexiconFirstResults(ctsQuery, sortType);
      case 'semantic-first':
        return this._getSemanticFirstResults(ctsQuery, sortType);
      default:
        return this._getDocumentFirstResults(ctsQuery, sortType);
    }
  }

  // Determine optimal plan construction based on search characteristics
  _selectOptimalPlanStrategy(sortType, ctsQuery) {
    // Optimization: Use lexicon-first when we have good field sorts and reasonable selectivity
    if (
      sortType === SORT_TYPE_MULTI_SCOPE &&
      this._isHighlySelective(ctsQuery)
    ) {
      return 'lexicon-first';
    }

    // Optimization: Use semantic-first when triple join selectivity is high
    if (sortType === SORT_TYPE_SEMANTIC && this._hasSelectivePredicates()) {
      return 'semantic-first';
    }

    // Default: Document-first (current approach)
    return 'document-first';
  }

  // Optimized lexicon-first approach for multi-scope sorts
  _getLexiconFirstResults(ctsQuery, sortType) {
    const { order, subSortConfigs } =
      this.sortCriteria.getMultiScopeSortOption();

    // Start with lexicon data and join back to documents
    const lexiconPlan = op
      .fromLexicons(
        {
          sortByMe: cts.fieldReference(subSortConfigs[0].indexReference),
          fragmentId: cts.fragmentIdReference(),
        },
        ctsQuery,
      )
      .joinDocUri('uri', op.fragmentIdCol('fragmentId'))
      .orderBy(
        order === 'ascending' ? op.asc('sortByMe') : op.desc('sortByMe'),
      );

    return this._getUnifiedPaginationResults(lexiconPlan);
  }

  // Original document-first approach with optimizations
  _getDocumentFirstResults(ctsQuery, sortType) {
    const searchOptions = this._buildOpticSearchOptions(sortType);

    // 1. Start with Optic search but optimize the initial projection
    let searchPlan = op
      .fromSearch(ctsQuery, ['fragmentId'], null, searchOptions)
      .joinDocUri('uri', op.fragmentIdCol('fragmentId'))
      .joinDoc(op.col('doc'), op.col('uri'))
      .select(['uri', 'fragmentId', op.xpath('doc', '/json/type').as('type')]); // Optimization: Get type early

    // 2. Conditionally enrich with sort-specific data
    searchPlan = this._enrichPlanWithSortData(searchPlan, sortType);

    // 3. Apply ordering
    searchPlan = this._applyPlanSorting(searchPlan, sortType);

    // 4. Unified pagination (now with type already available)
    return this._getOptimizedPaginationResults(searchPlan);
  }
  ...
  _getOpticPaginatedResults(opticPlan, order) {
    let finalPlan = opticPlan;

    // Only add ordering if not already applied and order is specified
    if (order) {
      finalPlan = finalPlan.orderBy(
        order === 'ascending'
  ...
  // Build search options for op.fromSearch based on sort type
  _buildOpticSearchOptions(sortType) {
    const baseOptions = { scoreMethod: 'simple' };

    if (sortType === SORT_TYPE_NON_SEMANTIC) {
      // Replicate the search options from _getNonSemanticSortResults
      ...
      // Convert array to object format for op.fromSearch
      return {
        ...baseOptions,
        searchOptionsArray: searchOptionsArr,
      };
    }
  ...
  // Enrich the base search plan with sort-specific data
  _enrichPlanWithSortData(searchPlan, sortType) {
    if (sortType === SORT_TYPE_MULTI_SCOPE) {
      return this._enrichWithMultiScopeData(searchPlan);
    } else if (sortType === SORT_TYPE_SEMANTIC) {
      return this._enrichWithSemanticData(searchPlan);
    }
    // Non-semantic doesn't need enrichment - sorting handled by search options
    return searchPlan;
  }

  // Add multi-scope sort data (extracted from _getMultiScopeSortResults)
  _enrichWithMultiScopeData(searchPlan) {
    const { subSortConfigs } = this.sortCriteria.getMultiScopeSortOption();
    const subSortPlans = subSortConfigs.map((subSortConfig) =>
      this._getSubSortPlan(subSortConfig),
    );
    ...
    return searchPlan.joinLeftOuter(
      unionSubSortPlan,
      op.on('fragmentId', 'fragmentId'),
    );
  }

  // Add semantic sort data (extracted from _getSemanticSortResults)
  _enrichWithSemanticData(searchPlan) {
    xdmp.setRequestTimeLimit(SEMANTIC_SORT_TIMEOUT);

    const { predicate, indexReference, order } =
      this.sortCriteria.getSemanticSortOption();
    const oneSortValuePerResult = true; // Could be parameterized later
    ...
    let enrichedPlan = searchPlan.joinLeftOuter(triplePlan);

    // Add indexed field data
    const indexedFieldPlan = op.fromLexicons({
      fieldDocIri: cts.iriReference(),
      sortByMe: cts.fieldReference(indexReference),
    });

    enrichedPlan = enrichedPlan.joinLeftOuter(
      indexedFieldPlan,
      op.on('objectIri', 'fieldDocIri'),
    );

    // Group for deduplication if needed
    if (oneSortValuePerResult) {
      enrichedPlan = enrichedPlan.groupBy(
        ['uri'],
        [
          order === 'ascending'
            ? op.min('sortByMe', op.col('sortByMe'))
  ...
  // Support for transitive search using op.fromSPARQL (initial implementation)
  _getTransitiveResults(startingIris, predicates, maxDepth = 3) {
    const sparqlQuery = `
      PREFIX crm: <http://www.cidoc-crm.org/cidoc-crm/>
      PREFIX la: <https://linked.art/ns/terms/>
      
      SELECT DISTINCT ?result WHERE {
        VALUES ?start { ${startingIris.map((iri) => `<${iri}>`).join(' ')} }
        ?start ${predicates.map((pred) => `<${pred}>`).join('|')}{1,${maxDepth}} ?result .
      }
    `;

    return op.fromSPARQL(sparqlQuery);
  }

  // Apply sorting based on sort type
  _applyPlanSorting(searchPlan, sortType) {
    if (sortType === SORT_TYPE_MULTI_SCOPE) {
      const { order } = this.sortCriteria.getMultiScopeSortOption();
      return searchPlan.orderBy(
        order === 'ascending'
          ? op.asc(op.col('sortByMe'))
          : op.desc(op.col('sortByMe')),
      );
    } else if (sortType === SORT_TYPE_SEMANTIC) {
      const { order } = this.sortCriteria.getSemanticSortOption();
      return searchPlan.orderBy(
        order === 'ascending'
          ? op.asc(op.col('sortByMe'))
          : op.desc(op.col('sortByMe')),
      ...
      // Non-semantic sorting is handled by search options
      // But we may need to add explicit ordering for field sorts
      const nonSemanticSortOption =
        this.sortCriteria.getNonSemanticSortOptions();
      if (nonSemanticSortOption && nonSemanticSortOption !== 'relevance') {
        // Would need to parse the sort option and add appropriate orderBy
        // This is where you'd add field-based sorting for non-semantic
      }
      return searchPlan;
    }
  }

  // Unified pagination for all sort types
  _getUnifiedPaginationResults(opticPlan) {
    if (this.pageWith) {
      return this._getOpticPageWithResults(opticPlan, null); // order handled in _applyPlanSorting
    } else {
      return this._getOpticPaginatedResults(opticPlan, null); // order handled in _applyPlanSorting
    }
  }
